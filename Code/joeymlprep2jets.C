...
void Jet_ML_Prep(char* file_name, char* output_tree_description, float pt_min, float pt_max, int i) {
  ...
  for ( int e = 0 ; e < combJet_tree->GetEntries() ; e++ ) {
	...
	float dy = pow(p_jet_y[k]-c_jet_y[j],2);
	float dphi1;
	if(c_jet_phi[j] > p_jet_phi[k])
	  {
	    dphi1 = pow(c_jet_y[j]-p_jet_y[k]-2*3.1415,2);
	  }
	else
	  {
	    dphi1 = pow(p_jet_y[k]-c_jet_y[j]-2*3.1415,2);
	  }
	float dphi2 = pow(p_jet_y[k]-c_jet_y[j],2);
	float dphi;
	if(dphi2 < dphi1)
	  {
	    dphi = dphi2;
	...
	const_total_pt += c_jet_const_pt[j][p];
      ...
      if((c_jet_pt[j] > 200) || (c_jet_pt[j] < 0))
	{
	  jet_pt_raw[j] = 0;
	}
      else
